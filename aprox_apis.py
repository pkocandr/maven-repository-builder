...
class AproxApi(UrlRequester):
    ...
    def urlmap(self, wsid, sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds, injectedBOMs,
        ...
        """
        cached = self.get_cached_urlmap(sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds,
        ...
        if cached:
            ...
            response = self.urlmap_response(wsid, sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds,
                                            injectedBOMs, resolve)
            if response != "{}":
                self.store_urlmap_cache(response, sourceKey, gavs, addclassifiers, excludedSources, preset,
    ...
    def paths(self, wsid, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve=True):
        """
        See paths_response() for method docs. This is wrapping method to the one with caching.
        """
        cached = self.get_cached_paths(sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve)
        if cached:
            ...
            response = self.paths_response(wsid, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve)

            self.store_paths_cache(response, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve)
    ...
    def paths_nocache(self, wsid, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve=True):
        """
    ...
    def paths_response(self, wsid, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs,
                       resolve=True):
        """
    ...
    def get_cached_urlmap(self, sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds,
                          injectedBOMs, resolve):
        """
        ...
        """
        cache_filename = self.get_urlmap_cache_filename(sourceKey, gavs, addclassifiers, excludedSources, preset,
    ...
    def store_urlmap_cache(self, response, sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds,
                           injectedBOMs, resolve):
        """
    ...
    def get_urlmap_cache_filename(self, sourceKey, gavs, addclassifiers, excludedSources, preset, patcherIds,
                                  injectedBOMs):
        """
        ...
        :param injectedBOMs: list of injected BOMs used with dependency management injection
                             Maven extension
        """
        cache_filename = "%s_|_%s_|_%s_|_%s_|_%s_|_%s_|_%s" % ("_".join(gavs), sourceKey, addclassifiers,
                                                               "_".join(excludedSources), preset, "_".join(patcherIds),
                                                               "_".join(injectedBOMs))
    ...
    def get_cached_paths(self, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve):
        """
    ...
    def store_paths_cache(self, response, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs, resolve):
        """
    ...
    def get_paths_cache_filename(self, sourceKey, roots, targets, excludedSources, preset, patcherIds, injectedBOMs):
        """
        ...
        :param injectedBOMs: list of injected BOMs used with dependency management injection
                             Maven extension
        """
        cache_filename = "%s_|_%s_|_%s_|_%s_|_%s_|_%s_|_%s" % ("_".join(roots), "_".join(targets), sourceKey, 
                                                               "_".join(excludedSources), preset, "_".join(patcherIds),
                                                               "-".join(injectedBOMs))
